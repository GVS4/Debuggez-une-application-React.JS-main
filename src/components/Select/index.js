...
const Select = ({
  ...
  const changeValue = (newValue) => {
    // Ajout : Appelle la fonction onChange avec la nouvelle valeur
    // La valeur est passé en prop par Events
    onChange(newValue);
    // Met à jour la valeur sélectionnée dans l'état local
    setValue(newValue);
  ...
  return (
    <div className={`SelectContainer ${type}`} data-testid="select-testid">
      {/* Affiche le label si elle est fournie */}
      {label && <div className="label">{label}</div>}
      <div className="Select">
        <ul>
          {/* Affiche la valeur sélectionnée ou "Toutes" si aucune valeur n'est sélectionnée */}
          <li className={collapsed ? "SelectTitle--show" : "SelectTitle--hide"}>
            {value || (!titleEmpty && "Toutes")}
          </li>
          {/* Affiche les options si le menu n'est pas réduit */}
          {!collapsed && (
            <>
              ...
              {/* Mappe les options disponibles et crée un élément pour chaque option */}
              {selection.map((s) => (
                <li key={s} onClick={() => changeValue(s)}>
                  <input
                    defaultChecked={value === s} // Vérifie si l'option est celle sélectionnée
                    name="selected"
                    type="radio"
                  />{" "}
                  {s}
        ...
        {/* Input caché pour soumettre la valeur sélectionnée avec le formulaire */}
